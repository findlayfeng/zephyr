...
struct zffs_data {
	...
	sys_slist_t opened;
	u8_t area_num;
	struct zffs_area base_areas[ZFFS_CONFIG_AREA_MAX];
	struct zffs_area *data_areas[ZFFS_CONFIG_AREA_MAX];
	struct zffs_area *tree_areas[ZFFS_CONFIG_AREA_MAX];
	struct zffs_area **swap;
	union zffs_addr next_tree;
	union zffs_addr next_data;
	union zffs_addr next_swap;
	u32_t next_id;
};
