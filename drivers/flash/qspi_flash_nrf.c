...
static int erase(off_t addr, size_t len)
{
	...
	if (addr & (KB(4) - 1) || len & (KB(4) - 1)) {
		return -EINVAL;
	}

	while (len > 0) {
		erase_len = (len >= KB(64) && (addr & (KB(64) - 1)) == 0) ?
			    NRF_QSPI_ERASE_LEN_64KB : NRF_QSPI_ERASE_LEN_4KB;

		err = nrfx_qspi_erase(erase_len, addr);
		...
		addr += erase_len == NRF_QSPI_ERASE_LEN_64KB ? KB(64) : KB(4);
		len -= erase_len == NRF_QSPI_ERASE_LEN_64KB ? KB(64) : KB(4);
	}
